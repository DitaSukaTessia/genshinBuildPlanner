...
REQUEST_HEADERS = {
    "User-Agent": "GenshinBuildPlanner/0.2 (+https://example.local)",
    "Accept": "application/json",
}

STATUS_CODE_DETAILS = {
    400: "Invalid UID format.",
    404: "UID not found on the game server.",
    424: "Enka or the game servers appear to be under maintenance.",
    429: "Rate limited by Enka.Network. Please try again shortly.",
...
async def fetch_uid_showcase(uid: str) -> dict[str, Any]:
    ...
    url = f"{ENKA_API_BASE_URL}/{normalized_uid}"
    ...
    if not raw_data.get("avatarInfoList"):
        raise HTTPException(
            status_code=404,
...
async def fetch_and_parse_uid(uid: str) -> dict[str, Any]:
