...
export function CharacterFocus({ character }) {
  return (
    <section className="overflow-hidden rounded-[2rem] border border-white/[0.06] bg-[#171b2d]/90 shadow-glow backdrop-blur">
      <div className="grid gap-4 p-4 lg:grid-cols-[minmax(0,1.15fr)_minmax(260px,0.85fr)]">
        <div className="overflow-hidden rounded-[1.5rem] border border-white/[0.06] bg-gradient-to-br from-[#222742] via-[#111629] to-[#0a0d18]">
          <img
            alt={character.name}
            className="h-full min-h-[320px] w-full object-cover object-top sm:min-h-[360px]"
            onError={(event) => {
        ...
        <div className="grid gap-4 md:grid-cols-[minmax(0,1fr)_120px] lg:grid-cols-[minmax(0,1fr)_110px]">
          <div className="flex flex-col gap-4">
            <div className="rounded-[1.5rem] border border-white/[0.06] bg-white/[0.04] p-4 sm:p-5">
              <p className="text-[11px] uppercase tracking-[0.28em] text-[#b9bed6]/55">Character Focus</p>
              <h2 className="mt-2.5 text-3xl font-semibold tracking-tight text-white sm:text-[2rem]">{character.name}</h2>
