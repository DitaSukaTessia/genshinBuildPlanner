...
export function WeaponCard({ weapon }) {
  ...
  return (
    ...
    >
      ...
      <div className="mt-3 grid gap-2.5 sm:mt-4 sm:gap-3 lg:grid-cols-[minmax(0,1.2fr)_minmax(0,0.8fr)_minmax(0,0.8fr)]">
        ...
        <div className="rounded-[1.3rem] border border-white/[0.06] bg-white/[0.04] p-3.5 sm:p-3.5">
          <p className="text-[11px] uppercase tracking-[0.2em] text-[#b9bed6]/55">Level</p>
          <p className="mt-2 text-[1.7rem] font-semibold text-white sm:mt-2.5 sm:text-[2rem]">{weapon.level ?? '—'}</p>
        ...
        >
          ...
          <p className="mt-2 text-[1.7rem] font-semibold text-white sm:mt-2.5 sm:text-[2rem]">R{weapon.refinement ?? '—'}</p>
