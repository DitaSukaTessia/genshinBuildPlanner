...
export function WeaponCard({ weapon }) {
  ...
  return (
    <section
      className="relative rounded-[2rem] border border-white/10 bg-[#171b2d]/90 p-5 shadow-glow backdrop-blur"
    ...
    >
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-[minmax(0,1.2fr)_minmax(0,0.8fr)_minmax(0,0.8fr)]">
        <div className="flex items-center gap-4 rounded-[1.5rem] border border-white/8 bg-white/[0.04] p-4">
          <img
            alt={weapon.name}
            className="h-20 w-20 rounded-[1.35rem] border border-white/10 object-cover"
            onError={(event) => {
        ...
        <div className="rounded-[1.5rem] border border-white/8 bg-white/[0.04] p-4">
          ...
          <p className="mt-3 text-3xl font-semibold text-white">{weapon.level ?? '—'}</p>
          <p className="mt-2 text-sm text-slate-400">Hover or click the detail tile for stat breakdown.</p>
        </div>

        <button
          className="rounded-[1.5rem] border border-white/8 bg-white/[0.04] p-4 text-left transition hover:border-star/30 hover:bg-white/[0.07]"
          onBlur={() => setShowStats(false)}
          onClick={() => setShowStats((current) => !current)}
          type="button"
        >
          <p className="text-[11px] uppercase tracking-[0.2em] text-[#b9bed6]/55">Refinement</p>
          <p className="mt-3 text-3xl font-semibold text-white">R{weapon.refinement ?? '—'}</p>
          <p className="mt-2 text-sm text-slate-400">Open detailed weapon stats</p>
        </button>
      </div>

      {showStats ? (
        <div className="absolute inset-x-5 top-[calc(100%+0.75rem)] z-10 rounded-[1.5rem] border border-white/10 bg-abyss/95 p-4 shadow-2xl xl:left-auto xl:right-5 xl:w-[340px]">
